...
class CargueMasivoController extends Controller
{
    ...
    public function descargarExcel($id)
    {
        ...
        $hoja->setCellValue("D1", "Abstract");
        $hoja->setCellValue("E1", "Palabras Clave");
        $hoja->setCellValue("F1", "Categoría");
        $hoja->setCellValue("G1", "Línea de Investigación");
        $hoja->setCellValue("H1", "Editorial");
        $hoja->setCellValue("I1", "Fecha de Publicación (YYYY/MM/DD)");
        $hoja->setCellValue("J1", "Lugar Publicación");
        $hoja->setCellValue("K1", "Autores (NombresAutor1, ApellidosAutor1 | NombresAutor2, ApellidosAutor2)");
        foreach ($listaDetalleCargue as $detalleCargue) {
            ...
            $this->armarCeldaExcel($hoja, 'E'.$numCelda, "texto", null, "Las palabras deben ir separadas por ,");
            $this->armarCeldaExcel($hoja, 'F'.$numCelda, "listaPersonalizada", 'Listas!$A$2:$A$'.$numCeldaCateg);
            $this->armarCeldaExcel($hoja, 'G'.$numCelda, "listaPersonalizada", 'Listas!$B$2:$B$'.$numCeldaLineaInves);
            $this->armarCeldaExcel($hoja, 'I'.$numCelda, "fecha");
            $this->armarCeldaExcel($hoja, 'J'.$numCelda, "listaPersonalizada",'Listas!$C$2:$C$'.$numCeldaLugPub);
            $this->armarCeldaExcel($hoja, 'K'.$numCelda, "texto",null,"Los Autores deben ir separados por un | nombres y apellidos separados por coma");
        }
        $hoja->getProtection()->setSheet(true);
        $hoja->getStyle('C2:K'.$numCelda)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);
